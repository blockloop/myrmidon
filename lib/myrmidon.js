(function(){
	...
	exports.downloadItem = function(info) { // title, author, id
		...
		var newArgs = [];
		var isAudioOnly = _.find(config.user.audioAuthors, function(item) {
			return item.toLowerCase() === info.author.toLowerCase();
		});
		if (isAudioOnly) {
			newArgs.push('--extract-audio');
		}

		var videoUrl = util.getVideoUrl(info.id);

		var dl = youtubedl.download(videoUrl, downloadDir, config.youtubeDlArgs.concat(newArgs));
		...
		dl.on('error', function(err) {
			console.error('Cannot download'.red);
			throw err;
		});
	...
	exports.downloadNewSubscriptions = function(user, limit) {
		...
		var data = http.get(url)
			.on('fail', function(err){
				...
				var msg = ('Could not access this user' + ' ('+ code +')').red;
